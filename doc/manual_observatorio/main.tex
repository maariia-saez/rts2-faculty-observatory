...
\begin{itemize}
    ...
    \item \textbf{dome} (Dome Daemon): Controla el movimiento de la cúpula, asegurándose de que la apertura (la rendija) esté siempre alineada con donde apunta el telescopio. El sistema hardware de control de nuestra cúpula es el \textbf{Talon 6}.
    \item \textbf{camd} (Camera Daemon): Se encarga de controlar la cámara principal, gestionar los tiempos de exposición y la refrigeración. \textit{(Actualmente el hardware fotográfico está pendiente de instalación)}.
...
El Meade LX200 se comunica con el ordenador mediante un adaptador USB a RS232. En entornos Windows se correspondía históricamente con el puerto \texttt{COM3}, lo cual en Linux se traducirá habitualmente en el dispositivo \texttt{/dev/ttyUSB0} (o \texttt{ttyUSB1}).
...
(Aquí se añadirán las líneas de configuración exactas una vez se configure el driver).
